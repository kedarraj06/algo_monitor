...
def predict(features_dict: dict) -> tuple:
    ...
    try:
        # Build ordered list of feature values
        feature_values = [features_dict[name] for name in feature_names]
    except KeyError as e:
        raise ValueError(f"Missing expected feature: {e}")

    # ── FIX: Pass a DataFrame with column names instead of a raw numpy array ──
    # The RandomForestClassifier was trained with named columns (pandas DataFrame).
    # Passing a raw numpy array triggers the UserWarning about missing feature names.
    # Wrapping in a single-row DataFrame with the exact column names eliminates it.
